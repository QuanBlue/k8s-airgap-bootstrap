...
| Backup scripts | `/opt/k8s-backup/scripts/backup` | `/u01/app/scripts/backup` |
...
backup:
  enabled: true
  scripts_dir: "/u01/app/scripts/backup"
...
wizard (default **on**). It deploys and executes the source-of-truth
role-specific scripts under `scripts/servers/iptables/`, one for masters and
one for workers. These scripts rebuild dedicated inbound and outbound chains
for the host while leaving the cluster dataplane tables and Calico-generated
chains in place.

**Source of truth**
- Firewall behavior, rule order, comments, and chain structure live in the two
  scripts under `scripts/servers/iptables/`
- `inventories/group_vars/all.yml` is the source of truth for cluster config
  values such as CIDRs, VIPs, Prometheus IPs, Teleport IPs, NTP IPs, SOC IPs,
  and external DB IPs
- `./scripts/bootstrap.sh` only synchronizes those values into the
  bootstrap-managed variable block at the top of each script

**The scripts trust traffic from:**
- loopback, `ESTABLISHED,RELATED`, ICMP
- the rendered cluster CIDRs for node, pod, and service traffic
- Calico transport from the rendered node CIDR only (`4789/udp`, `179/tcp`,
  `5473/tcp`, `51820-51821/udp`)
- the rendered VIP, Prometheus, Teleport, NTP, SOC, and database endpoint data
  from the bootstrap config

The managed chains are inserted first in `INPUT` and `OUTPUT`, so their
allowlist and final `DROP` run before later host-level rules, including any
Calico rules in those base chains. The scripts intentionally do not flush or
edit `FORWARD`, `nat`, `mangle`, or `cali-*`; those remain owned by
Kubernetes, kube-proxy, and Calico.

**Then they `DROP` sensitive ports when traffic falls outside the trusted rules:**

| Node | Dropped from outside the cluster | Always open |
|---|---|---|
| master | `6443` (API), `2379-2380` (etcd), `10257` (c-m), `10259` (sched), `10250` (kubelet) | `22` (SSH), `8443` (HAProxy → API), VRRP from node CIDR |
| worker | `10250` (kubelet), `10256` (kube-proxy) | `22` (SSH), optional worker VIP `80/443/9090`, Calico/BGP/WireGuard from node CIDR, rendered service backends |
...
firewall:
  enabled: true
  scripts_dir: "/u01/app/scripts/iptables"
...
The server-side iptables scripts under `scripts/servers/iptables/` are the
source of truth for firewall application. During the firewall play, the
node-specific script is copied onto each node under `firewall.scripts_dir`
(for example `/u01/app/scripts/iptables`) and executed directly. The bootstrap
wizard updates only the bootstrap-managed value block near the top of each
script, preserving manual edits to firewall logic below that block.

Prometheus scrapers are modeled as inbound allowlist traffic to TCP
`9000:9300`. Teleport is modeled as outbound-only agent traffic; if enabled in
bootstrap, masters and workers may connect to the rendered Teleport Proxy/Auth
IPs, defaulting to `10.129.0.232`, on fixed TCP ports `443`, `3080`, and
`3024`. NTP can optionally be restricted to rendered server IPs; otherwise the
scripts keep the generic UDP `123` fallback for compatibility. No inbound
Teleport ports are opened on Kubernetes nodes.

For manual custom rules and chain management, see
[`iptables-customization.md`](./iptables-customization.md).
